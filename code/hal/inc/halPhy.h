...
INLINE uint8_t HAL_PhySpiWriteByteInline(uint8_t value) {
    SPI2BUFL = value;
    while (!SPI2STATLbits.SRMT);        // SRMT -> Shift Register Empty, 1 hogyha már nincsen adat amit küldeni kell
    
    return SPI2STATLbits.FRMERR;        // Az AVR-es implementációnál a return a buffert adja vissza (ahhoz kell, hogy egy Write Collision bit clearelve
                                        // legyen), itt én csak egy error bitet küldök vissza (Frame Error)
}
